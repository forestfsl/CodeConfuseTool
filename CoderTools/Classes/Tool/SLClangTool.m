...
/** 类名、方法名 */
@interface SLTokensClientData : NSObject
@property (nonatomic, strong) NSArray *prefixes;
@property (nonatomic, strong) NSMutableSet *tokens;
@property (nonatomic, copy) NSString *file;
@end

@implementation SLTokensClientData
@end

/** 字符串 */
@interface SLStringsClientData : NSObject
@property (nonatomic, strong) NSMutableSet *strings;
@property (nonatomic, copy) NSString *file;
@end

@implementation SLStringsClientData
@end

@implementation SLClangTool
...
enum CXChildVisitResult _visitTokens(CXCursor cursor,
    ...
    SLTokensClientData *data = (__bridge SLTokensClientData *)clientData;
...
enum CXChildVisitResult _visitStrings(CXCursor cursor,
    ...
    SLStringsClientData *data = (__bridge SLStringsClientData *)clientData;
    if (!_isFromFile(data.file.UTF8String, cursor)) return CXChildVisit_Continue;
...
{
    SLStringsClientData *data = [[SLStringsClientData alloc] init];
    data.file = file;
...
+ (NSSet *)classesAndMethodsWithFile:(NSString *)file
                          ...
                          searchPath:(NSString *)searchPath
{
    SLTokensClientData *data = [[SLTokensClientData alloc] init];
    data.file = file;
               ...
               clientData:(CXClientData)clientData
{
    ...
    if (searchPath.length) {
        subDirs = [NSFileManager sl_subdirsAtPath:searchPath];
