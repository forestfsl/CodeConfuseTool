...
{
    if (filepath.sl_stringByRemovingSpace.length == 0 || !completion) return;
...
{
    if (filename.sl_stringByRemovingSpace.length == 0 || !completion) return;
...
{
    if (dir.sl_stringByRemovingSpace.length == 0) return nil;
...
+ (NSString *)sl_checkPathExists:(NSString *)path
{
    ...
    while ([mgr fileExistsAtPath:filepath]) {
        [NSFileManager sl_divideFilename:filepath completion:^(NSString *filename, NSString *extension) {
